...
8c3. **[`.cursor/rules/alchemist-python-economy.mdc`](.cursor/rules/alchemist-python-economy.mdc)** — **local Python** (`~/alchemist-tools/`) for bulk data/fixtures so assistants **do not burn tokens** generating JSON inline; **not** a substitute for TS gates, **`pnpm learning:*`**, or **`tools/validate-offsets.py`** (HARD GATE)  
