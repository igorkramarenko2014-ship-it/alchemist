...
9. **[`docs/brain.md`](docs/brain.md)** — **§14** human-readable map (incl. RU); **§9a–§9b** **brain-fusion** calibration (`pnpm brain:sync` → **`packages/shared-engine/brain-fusion-calibration.gen.ts`**) + **§9b golden rule** (fused chat / agent-aji layer as project **soul** / ethos — **not** an override of FIRESTARTER, HARD GATE, or DSP-vs-TS facts). **[`docs/brain-plus.md`](docs/brain-plus.md)** — metrics shell — same inner-circle **detail** in skill folder  
