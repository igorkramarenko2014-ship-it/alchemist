...
| Wrapper domain bootstrap | `wrapper/engine-wrapper.ts`, `wrapper/wiki-bridge.ts` | **WikiBridge** runs once on wrapper init per domain context, fetches **Wikipedia lead summaries only**, builds structured **`WikiKnowledgeBase`**, caches by **`wiki:{domain}:{language}`** (default **24h**), injects vocabulary/concepts into Transmutation, and **fails open**; it does **not** call the triad directly or inject raw article text into prompts. |
