...
| **Corpus filesystem** | Under **`corpus/`**, **only** lesson **`*.json`**, optional **`*.md`**, **`.gitkeep`**, and optional root **`corpus/.gitignore`** (ignore rules for accidental binaries) — **any other file** triggers cleanup then re-check: **`validate-learning-corpus.mjs`** runs **`scripts/learning-forget-presets.mjs`** once (same as **`pnpm learning:forget-presets`**), then rescans. To **fail closed** without auto-deletion (tests / debug): **`LEARNING_CORPUS_NO_AUTO_SANITIZE=1`**. Manual fix: **`pnpm learning:sanitize`**. Emergency skip of the FS allowlist entirely: **`LEARNING_CORPUS_SKIP_FS_CLEAN_CHECK=1`**. |
| **Schema** | **`packages/shared-engine/learning/schema/lesson.schema.json`** (JSON Schema draft 2020-12). Extension field **`x-alchemist-schema-version`** is **`1.2`** and must match each lesson’s required **`schemaVersion`** (`"1.1"` or **`"1.2"`**). Schema **1.2** adds optional **`cluster`** (string tag for affinity / ops). Optional **`antiPatterns`**, **`difficulty`**, **`heuristics`**, **`contrastMatrix`**, **`lessonVersion`**, **`changelog`**. |
...
**Authoring hygiene:** When creating **large** new lesson JSON or batch-editing corpus files, prefer **local Python** scripts (**`~/alchemist-tools/`**, **`.cursor/rules/alchemist-python-economy.mdc`**) over pasting huge blobs into an LLM — same validation gates apply once files land under **`corpus/`**.
...
```json
{"status":"ok","validatedFiles":1,"schemaVersion":"1.2","minLessons":1}
```

Failure (example shape):

```json
{"status":"fail","errors":[{"file":"packages/shared-engine/learning/corpus/bad.json","path":"/mappings","message":"..."}],"schemaVersion":"1.2","minLessons":1,"scannedFiles":2}
```
...
**Derived index (phase 1 — implemented).** **`pnpm learning:build-index`** runs **`packages/shared-engine/learning/scripts/build-learning-index.mjs`**, walks the same **`corpus/**/*.json`** tree, and writes **`packages/shared-engine/learning/learning-index.json`** (currently **gitignored**). The file holds **`generatedAtUtc`**, **`schemaVersion`**: **`"1.2"`** (index payload), **`lessonCount`**, and per-lesson summaries (**`id`**, **`style`**, truncated **`character`** / **`causalReasoning`**, **`tags`**, **`mappingKeys`**, optional **`cluster`**, optional **`priorityMappingKeys`**, **`coreRules`**, **`contrastWith`**, optional **`difficulty`**, **`lessonVersion`**, **`antiPatternCount`**, **`contrastMatrixVs`**, optional **`fitnessScore`** when **`artifacts/learning-fitness-report.json`** exists — **no** full **`mappings`** values). **Stdout:** one JSON line, e.g. `{"status":"ok","lessonCount":N,"outputPath":"packages/shared-engine/learning/learning-index.json"}`; **exit 1** on failure. **CI** does not require a fresh index today; **`pnpm learning:verify`** remains the **authoritative** gate for committed lessons.
...
**Phase 2 (shipped — opt-in).** **`ALCHEMIST_LEARNING_CONTEXT=1`** + built index → **`POST /api/triad/*`** appends a bounded (**≤800** characters for the full block) **advisory** context. Selection is **deterministic**: sort prefers **cluster** token overlap → **`priorityMappingKeys`** → **`coreRules`** → **`antiPatternCount`** tie-break, then composite score; meaningful tokens, stopwords, tag/style/mappingKey scoring with mappingKey **+0.5**; dedupe by style + overlapping tags; **default up to 2** lessons per run (override via selection options). Compact lines may include **`[cluster: …]`** when present — not full lesson blobs. Telemetry: **`learningContextUsed`** on **`triad_run_start`** (`injected`, `selectedLessonIds`, **`contextCharCount`**, optional **`selectedClusters`**). **`triadSessionId`** — one id per **`runTriad`** session — is accepted on the JSON body of **`POST /api/triad/*`** and echoed on **`triad_run_end`** / structured JSONL (**`EngineSchoolTelemetryRecord`**) so three panelist calls and Engine School rows align in logs. With learning context on, **`engine_school_influence`** on stderr (**`logEvent`**) follows **`apps/web-app/env.ts`**: default **on** in development and **Vercel preview**, off in production and **`NODE_ENV=test`** unless **`ALCHEMIST_LEARNING_TELEMETRY=1`**; **`=0`** forces off. Payload adds **`panelistPipeline`** (raw → echo audit → **`isValidCandidate`**), **`bestScore`/`meanScore`**, **`selectedClusters`**, **`fullGatePipeline`:** **`client_runTriad`** — **advisory only**; Slavic/Undercover + merge still run client-side.

**Telemetry aggregation (JSONL → fitness, gitignored).** Optional append-only **`artifacts/learning-telemetry/*.jsonl`** (see **`ALCHEMIST_LEARNING_TELEMETRY_FILE`** / **`apps/web-app/env.ts`**) feeds **`pnpm learning:aggregate-telemetry`** → **`artifacts/learning-fitness-report.json`** (schema **`aggregationVersion`**: **3** — Fitness v1 per lesson: **`fitnessScore`**, **`fitnessConfidence`**, **`sampleCount`**, **`stalenessDays`**, non-authoritative **`learningOutcomes`** for AIOM display) + optional **`fitnessSnapshot`** merge into **`learning-index.json`**. Evidence is **panelist-route** (`isValidCandidate`) and **pre–client-gate** — auditable, does **not** read Slavic/Undercover outcomes from the browser. **`pnpm learning:assess-fitness`** runs **static v0** metadata (**`pipeline`: `static_metadata_v0`**) **then** aggregation. **`pnpm learning:forget-telemetry`** prunes old shards. **Not** part of fail-closed **`learning:verify`** unless you add it.

**`pnpm learning:assess-fitness`** first line prints **v0** corpus heuristics (anti‑patterns, contrast, optional **`cluster`** bump); the second step writes log-backed rows. **`pnpm learning:enrich-preview`** previews the block without enabling env.

**Phase 3 (shipped — opt-in).** **`ALCHEMIST_CORPUS_PRIOR=1`** + built index → **`scoreCandidates`** may apply a **corpus-affinity re-rank** after Slavic + intent blend (**Slavic cosine / Dice thresholds unchanged**; **no** Undercover edits). **`computeCorpusAffinity`** applies **`lessonEffectiveAffinityWeight`** (**`fitnessScore` × confidence**, bounded **0.05–0.15**, staleness **> 14** days reduces lift). Default corpus nudge weight **0.45**. Telemetry: **`score_candidates`** with **`corpusAffinityApplied`**, **`corpusAffinityOrderChanged`**, **`topCandidateScoreBefore`/`After`**, **`lessonIdsUsed`**, **`tasteClusterHit`** (null when taste pass inactive). Does **not** promote candidates that failed gates (re-rank is a **sort** of survivors only).

**Phase 4 — Taste Corpus (shipped — opt-in).** **`ALCHEMIST_TASTE_PRIOR=1`** + built **`taste-index.json`** (**`pnpm taste:build-index`**) → **`scoreCandidates`** applies a **`computeTasteAffinity`** advisory nudge after all gates pass. Governance hierarchy: **`1F7_FENOMAN`** (canonical anchor, own music) → **`ALPAKA_FAV`** (55-track lighthouse) → **`Amsterdammer_`** (1338-track eclectic corpus). Six taste clusters derived by K-means on Spotify audio features: `dark_alt_rock` (dominant — 0.95 own-music affinity in the shipped example index), `hard_driven_high_tempo`, `urban_groove`, `instrumental_electronic`, `euphoric_pop_dance`, `acoustic_melancholic`. **Global bias:** `darkValenceBias` — bright/acoustic candidates penalised. Default nudge weight **0.06** (lighter than Phase 3 corpus default **0.45**; taste is a smaller additive sort nudge). Does **not** promote candidates that failed gates. Does **not** touch Slavic/Undercover thresholds or blend weights. Telemetry: **`score_candidates`** with **`tasteAffinityApplied`**, **`tasteClusterHit`**, **`tasteEffectiveWeight`**. Rebuild from new Exportify CSVs: **`pnpm taste:rebuild`** (drops raw CSVs in `learning/taste/raw/`, gitignored). Validate schema: **`pnpm taste:validate`** — stdout `{"status":"ok","clusterCount":6}` / exit 1 on fail. **`taste-index.json`** gitignored; **`taste-schema.json`** committed + versioned (`schemaVersion: "1.0"`).
