...
1. **Self-healing registry (P0)** — Before inventing a new engine slice, read **`igor-power-cells.json`**. Run **`pnpm igor:heal`** (`scripts/igor-self-heal.mjs`) for a **read-only** scan: stderr **`iom_self_heal_proposal`** lists **ghost** `.ts` files not registered as cell artifacts (plus **`index.ts` / `telemetry.ts`** allowlist). If a **new cell** is justified, **do not** auto-edit the JSON in the agent. Emit an auditable line via **`logIomSelfHealProposal()`** (**`@alchemist/shared-engine`**) or paste the equivalent JSON for the operator, plus the exact **`{ id, responsibility, artifacts[] }`** snippet. **Headroom:** **`IOM_POLICY_CELL_MAX` = 12** (consolidation target). If **`currentCellCount` ≥ 12**, propose a **transmutation** (merge/retire cells) before adding. Machine sync may allow more rows until **`IOM_CELL_MAX`** is lowered — see **`docs/iom.md`**.
