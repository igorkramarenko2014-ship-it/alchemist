...
**Index:** Canonical **`docs/`** laws: **`FIRE.md`** (this file) + **`FIRESTARTER.md`**. **Supplementary:** **`brain.md`** (detailed project brain), **`brain-plus.md`** (minimal outside-assessment shell; metrics synced with **`pnpm fire:sync`**), **`inner-circle-agent.md`** (peer prompt + Canon FIREWALL — tone only). **`docs/alchemist-*.html`** — Composer prompt packs (**`FIRESTARTER` §12**). Optional multi-step agent flow: **`FIRESTARTER` Appendix C**. Root **`README.md`**, **`AGENTS.md`**, **`.cursorrules`**. **Legal:** **`LEGAL.md`**, **`PRIVACY.md`**, **`LICENSE`**, **`SECURITY.md`**.
...
- **Igor orchestrator (ops map, not governance):** **`packages/shared-engine/igor-orchestrator-layer.ts`** + **`pnpm igor:sync`** — discovers **`@alchemist/*`** workspaces, merges **`igor-orchestrator-meta.json`**, emits **`igor-orchestrator-packages.gen.ts`**; reads **`igor-power-cells.json`** → **`igor-orchestrator-cells.gen.ts`**. **`pnpm verify:harsh`** / **`harshcheck`** run **`sync-igor-orchestrator.mjs --check`** (after **`brain:sync` --check**). **`GET /api/health`** includes **`igorOrchestrator`** (full manifest). Optional **`logIgorOrchestratorManifest()`** → **`igor_orchestrator_manifest`** on stderr. **Does not** override gates, types, or triad math — see **`docs/brain.md` §9c–§9d**, **`AGENTS.md` §8e**.
- **Persona vs canon:** Peer tone / inner-circle prompt (**`docs/inner-circle-agent.md`**, **`inner-circle-voice`**) **never** overrides Serum **HARD GATE**, TS gate facts, or triad wiring. Typical discipline: **`pnpm verify:harsh`** → **`pnpm harshcheck`** → **`pnpm fire:sync`** (refresh metrics); **green CI / verify alone** ≠ healthy **`.fxp`/WASM** path — **§E1**.
